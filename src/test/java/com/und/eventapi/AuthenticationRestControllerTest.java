...
import com.und.model.security.AuthorityName;
...
public class AuthenticationRestControllerTest {
    ...
    public void successfulAuthenticationWithAnonymousUser() throws Exception {
        ...
        RestAuthenticationRequest restAuthenticationRequest = new RestAuthenticationRequest("user", "password");

        this.mvc.perform(post("/auth")
                .contentType(MediaType.APPLICATION_JSON)
                .content(new ObjectMapper().writeValueAsString(restAuthenticationRequest)))
    ...
    public void successfulRefreshTokenWithUserRole() throws Exception {
        ...
        UndUserDetails eventUser = RestUserFactory.create(user);

        when(this.restTokenUtil.getUsernameFromToken(any(), any())).thenReturn(user.getUsername());

        when(this.userDetailsService.loadUserByUsername(any())).thenReturn(eventUser);

        when(this.restTokenUtil.canTokenBeRefreshed(any(), any(), any())).thenReturn(true);
